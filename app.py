...
st.markdown('<div class="big-title">🎯 今日全商品推薦觀察榜單</div>', unsafe_allow_html=True)
...
@st.cache_data(ttl=3600)  # 快取 1 小時，最安全穩定
def fetch_and_analyze(ticker_name, ticker_symbol):
    try:
        ...
        # 【轉換成百分制分數】原始score範圍是 -6 到 +6，對應到 0 到 100 分
...
results = sorted(results, key=lambda x: x["score_100"], reverse=True)
